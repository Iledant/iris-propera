...
func getRatiosTest(e *httpexpect.Expect, t *testing.T) {
	...
	for _, tc := range testCases {
		...
		if tc.Status == http.StatusOK {
			response.JSON().Object().Value("PaymentRatio").Array().Length().Equal(tc.ArraySize)
		}
	}
}
...
func getPtRatiosTest(e *httpexpect.Expect, t *testing.T) {
	testCases := []testCase{
		...
		{Token: testCtx.User.Token, Status: http.StatusBadRequest,
			ID: "0", BodyContains: []string{"Liste des ratios : chronique introuvable"}},
		{Token: testCtx.User.Token, Status: http.StatusOK,
...
func setPtRatiosTest(e *httpexpect.Expect, t *testing.T) {
	testCases := []testCase{
		...
		{Token: testCtx.Admin.Token, ID: "0", Status: http.StatusBadRequest,
			BodyContains: []string{"Ratios : chronique introuvable"}},
...
func deletePtRatiosTest(e *httpexpect.Expect, t *testing.T) {
	testCases := []testCase{
		{Token: testCtx.User.Token, ID: "0", Status: http.StatusUnauthorized,
			BodyContains: []string{"Droits administrateur requis"}},
		{Token: testCtx.Admin.Token, ID: "0", Status: http.StatusBadRequest,
			BodyContains: []string{"Suppression de ratios : introuvable"}},
		{Token: testCtx.Admin.Token, ID: "5", Status: http.StatusOK,
...
func getYearRatiosTest(e *httpexpect.Expect, t *testing.T) {
	testCases := []struct {
		...
		{Token: "fake", Status: http.StatusInternalServerError,
			BodyContains: []string{"Token invalide"}},
		{Token: testCtx.User.Token, Status: http.StatusBadRequest,
			BodyContains: []string{"Ratios annuels : année manquante"}, ArraySize: 8},
		{Token: testCtx.User.Token, Status: http.StatusOK, Year: "2011",
	...
	for _, tc := range testCases {
		...
		response.Status(tc.Status)
		if tc.Status == http.StatusOK {
			response.JSON().Object().Value("Ratios").Array().Length().Equal(tc.ArraySize)
		}
	}
}
