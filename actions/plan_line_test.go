...
func getPlanLinesTest(e *httpexpect.Expect, t *testing.T) {
	...
	for i, tc := range testCases {
		...
		if tc.Status == http.StatusOK {
			response.JSON().Object().Value("PlanLine").Array().Length().Equal(tc.ArraySize)
		}
	}
}
...
func getDetailedPlanLinesTest(e *httpexpect.Expect, t *testing.T) {
	testCases := []testCase{
		...
		{Token: testCtx.User.Token, ID: "0", Status: http.StatusBadRequest,
			BodyContains: []string{"Liste détaillée des lignes de plan : plan introuvable"}, ArraySize: 59},
		{Token: testCtx.User.Token, ID: "1", Status: http.StatusOK,
	...
	for i, tc := range testCases {
		...
		response.ContentType("application/json")
		if tc.Status == http.StatusOK {
			response.JSON().Object().Value("DetailedPlanLine").Array().Length().Equal(tc.ArraySize)
		}
	}
}
...
func modifyPlanLineTest(e *httpexpect.Expect, t *testing.T, plID int) {
	testCases := []struct {
		...
		{Token: testCtx.Admin.Token, ID: "0", PlanLineID: "0", Status: http.StatusBadRequest, Sent: []byte(`{Plu}`),
			BodyContains: []string{"Modification de ligne de plan : plan introuvable"}},
		{Token: testCtx.Admin.Token, ID: "1", PlanLineID: "0", Status: http.StatusBadRequest, Sent: []byte(`{Plu}`),
...
func deletePlanLineTest(e *httpexpect.Expect, t *testing.T, plID int) {
	testCases := []struct {
		...
		{Token: testCtx.User.Token, ID: "0", PlanLineID: "0", Status: http.StatusUnauthorized, BodyContains: []string{"Droits administrateur requis"}},
		{Token: testCtx.Admin.Token, ID: "1", PlanLineID: "0", Status: http.StatusBadRequest,
