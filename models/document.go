...
func (d *Documents) GetOpAll(pID int64, db *sql.DB) (err error) {
	rows, err := db.Query(`SELECT id,physical_op_id,name,link FROM documents WHERE physical_op_id = $1`, pID)
	if err != nil {
		return err
	}
	var r Document
...
func (d *Document) Create(db *sql.DB) (err error) {
	err = db.QueryRow("INSERT INTO documents (physical_op_id,name,link) VALUES($1,$2,$3) RETURNING id",
...
func (d *Document) Update(db *sql.DB) (err error) {
	res, err := db.Exec(`UPDATE documents SET physical_op_id = $1, name = $2, link = $3 WHERE id = $4`,
