...
func (d *DetailedActionPayments) GetAll(year int64, ptID int64, db *sql.DB) (err error) {
	...
	(
	 (SELECT op.budget_action_id AS action_id, op.number, op.name, SUM(ct.y1) * 0.01 AS y1, SUM(ct.y2) * 0.01 AS y2, SUM(ct.y3) * 0.01 AS y3 FROM
			crosstab(
	 ...
	 UNION ALL
	 (SELECT action_id, NULL AS number, NULL AS name, SUM(y1) * 0.01 AS y1, SUM(y2) * 0.01 AS y2, SUM(y3) * 0.01 AS y3 FROM
			crosstab(
					'WITH pr AS (SELECT * FROM payment_ratios WHERE payment_types_id = ` + sptID + `),
